...
def create_classes_df(categories):
    """
    It parses and save the supercategories into classes.txt
    """
    #categories = [{'supercategory': 'CONE', 'id': 0, 'name': 'CONE'}, {'supercategory': 'CORNING16', 'id': 1, 'name': 'CORNING16'}, {'supercategory': 'HUAWEI1', 'id': 2, 'name': 'HUAWEI1'}, {'supercategory': 'HUAWEI2', 'id': 3, 'name': 'HUAWEI2'}, {'supercategory': 'HUAWEI3or4', 'id': 4, 'name': 'HUAWEI3or4'}, {'supercategory': 'OPT8', 'id': 5, 'name': 'OPT8'}, {'supercategory': 'OTHERCTO', 'id': 6, 'name': 'OTHERCTO'}, {'supercategory': 'UCA16', 'id': 7, 'name': 'UCA16'}]
    
    classes_df = pd.DataFrame()
    
    for cat in categories:
        classes_df = pd.concat([classes_df,pd.DataFrame([cat['name']],index=[cat['id']])])
        
    return classes_df


def save_dataset_yaml(classes_df, output_dir):
    """
    It saves both files: clases.txt and data.yaml
    """
    # Save classes file
    classes_df.to_csv(os.path.join(output_dir, 'classes.txt'), header=None, index=None)

    list_classes = classes_df.iloc[:,0].to_list()
