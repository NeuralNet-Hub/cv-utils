...
# python mask_the_face.py --path 'data' --mask_type 'all' --labels --verbose
...
parser.add_argument(
    ...
    dest="labels",
    action="store_true",
    help="If true, labels are copied with the name of new image in masked folder",
)
        ...
        for f in files:
            ...
            if is_image(image_path):
                ...
                for i in range(len(mask)):
                    ...
                    if args.labels:
                        orig_path_label=os.path.join(args.path,"labels",split_path[0]+".txt")
                        new_path_label=os.path.join(args.path+"_masked","labels",split_path[0]+"_"+mask[i]+".txt")
                        shutil.copy(orig_path_label,new_path_label)
