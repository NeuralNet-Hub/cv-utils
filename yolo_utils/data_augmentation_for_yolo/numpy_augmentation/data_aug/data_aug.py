...
class Rotate(object):
    ...
    def __call__(self, img, bboxes):
        ...
        if len(corners): # in case bboxes is empty
            corners[:,:8] = rotate_box(corners[:,:8], angle, cx, cy, h, w)
...
class RandomNoise(object):
    ...
    Parameters
    ----------
    Non parameter needed
    Returns
    -------
    numpy.ndaaray
        Noised image in the numpy format of shape `HxWxC`
    numpy.ndarray
        Tranformed bounding box co-ordinates of the format `n x 4` where n is
        number of bounding boxes and 4 represents `x1,y1,x2,y2` of the box
    """
    ...
    def __call__(self, img, bboxes):
        ...
        sigma = np.random.uniform(0.10,0.15,1) # The higher the more aggressive keep it between (0, 0.15)
...
class RandomBrightContrast(object):

    """
    Adjusts contrast and brightness of an uint8 image.
    contrast:   (0.0,  inf) with 1.0 leaving the contrast as is
        The higher the number the most contrast will be added (more white images)
        0 or near to 0 means completely gray
        5 is already a high number, so usually 3 is taken by default
    brightness: [-255, 255] with 0 leaving the brightness as is
        The higher the number the most brightness will be added (more white images)
        0 or near to 0 means non brightness modification (original image is returned)
        200 is already a high number, so usually 100 is taken by default  
    ...
    def __init__(self, max_contrast=3):
        self.max_contrast = max_contrast
    
    def __call__(self, img, bboxes):
        
        contrast = np.random.uniform(0,self.max_contrast,1).item()
